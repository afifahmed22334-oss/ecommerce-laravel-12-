                    ...
                    <div class="swiper-slide">
                        <div class="overflow-hidden position-relative h-100">
                            ...
                            <div class="character_markup type2">
                                <p class="text-uppercase font-sofia mark-grey-color animate animate_fade animate_btt animate_delay-10 mb-0">{{ $slider->tagline }}</p>
                            </div>
                            </div>
                        ...
                        @foreach ($sProducts as $sProduct)
                            <div class="swiper-slide product-card product-card_style3">
                                ...
                                <div class="pc__info position-relative">
                                    <h6 class="pc__title"><a href="{{ route('shop.show',$sProduct->slug) }}">{{ $sProduct->name }}</a></h6>
                                    <div class="product-card__price d-flex">
                                        <span class="money price text-secondary">
                                            @if ($sProduct->sale_price)
                                                ${{ $sProduct->sale_price }} | <s>${{ $sProduct->regular_price }}</s>
                                    ...
                                    <div class="anim_appear-bottom position-absolute bottom-0 start-0 d-none d-sm-flex align-items-center bg-body">
                                        @if (isset(session('cart')[$sProduct->id]))
                                            <a href="{{ route('cart.index') }}" class="btn-link btn-link_lg me-4 text-uppercase fw-medium">Go
                                                to cart</a>
                                        ...
                                        @else
                                            <form name="addtocart-form" method="POST" action="{{ route('cart.store') }}">
                                                ...
                                                <button type="submit" class="btn-link btn-link_lg me-4 text-uppercase fw-medium" title="Add To Cart">Add To
                                                    Cart</button>
                                            </form>
                                        @endif
                        ...
                        <div class="pc__info position-relative">
                            ...
                            <span class="money price text-secondary">
                                @if ($fProduct->sale_price)
                                    ${{ $fProduct->sale_price }} | <s>${{ $fProduct->regular_price }}</s>
                            ...
                            <div class="anim_appear-bottom position-absolute bottom-0 start-0 d-none d-sm-flex align-items-center bg-body">
                                @if (isset(session('cart')[$fProduct->id]))
                                    <a href="{{ route('cart.index') }}"
                                        class="btn-link btn-link_lg me-4 text-uppercase fw-medium">Go
                                        to cart</a>

                                @else
                                <form name="addtocart-form" method="POST" action="{{ route('cart.store') }}">
                                    @csrf
                                    <input type="hidden" name="product_id" value="{{ $fProduct->id }}">
                                    <input type="hidden" name="quantity" value="1" min="1">
                            ...
                            <a href="{{ route('shop.show', $fProduct->slug) }}" class="btn-link btn-link_lg me-4 text-uppercase fw-medium js-quick-view">
                                <span class="d-none d-xxl-block">Quick View</span>
                                <span class="d-block d-xxl-none"><svg width="18" height="18" viewBox="0 0 18 18" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <use href="#icon_view" />
